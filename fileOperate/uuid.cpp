...
void conInfoWriter::write(std::string_view info) const
{
    ...
    fs::rename(tmp,path);
    if(fs::exists(tmp)){std::cerr<<"1"<<std::endl;}
}
...
int main()
{
    conInfoWriter conInfo(acceptorName,requesterName,tag,rank,addressDirectory);
    conInfo.write("127.0.0.1:5005");
    return 0;
}